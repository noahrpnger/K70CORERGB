...
# Register 0x03 is the hardware lighting mode:
#   0x00 = default effect engine (firmware-controlled, reverts on iCUE close)
#   0x01 = software control mode (accepts direct color packets)
#   0x02 = transient/prep mode
#
# On exit, iCUE writes reg=0x03 val=0x00 to hand control back to firmware,
# which is why the keyboard reverts to stock lighting as soon as iCUE closes.
# We do NOT write 0x00 in DEINIT so the last colors set persist.


# The full "take software control" register sequence that iCUE sends before
# uploading config tables and colors. Without these writes — even with the
# config tables correct — the effect engine keeps running and overwrites
# every color frame with the default static-white pattern.
#
# Extracted from iCue_startUp.pcapng: all cmd=0x01 packets from t=6.1s to
# t=8.35s, preserving order. Values not fully decoded; reproduced verbatim.
_REGISTER_INIT: list[bytes] = [
    _reg_write(0x0003, 0x0002),   # mode: prep
    _reg_write(0x0003, 0x0001),   # mode: software control
    _reg_write(0x0003, 0x0002),   # mode: prep (again — iCUE cycles it)
...
INIT_PACKETS: list[bytes] = [
    # Preliminary probes (unchanged from original)
    ...
    # Register writes that disable the effect engine and hand control to software
    *_REGISTER_INIT,
    # Config tables the hardware requires before accepting color packets
...
# Intentionally does NOT write reg=0x03 val=0x00 (which is what iCUE sends on
# close to revert to hardware lighting). Leaving mode in software-control
# state means the last colors set remain after the app exits.
DEINIT_PACKETS: list[bytes] = [
    _pad(bytes([0x08, 0x05, 0x00])),
]
